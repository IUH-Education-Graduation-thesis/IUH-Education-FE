...
const ModalLichHoc = ({
  ...
  const optionsForNhomThucHanh = [
    ...Array(lopHocPhan?.soNhomThucHanh || 0)?.keys(),
  ]?.map((item) => ({
    label: `Nhóm ${item + 1}`,
    value: item + 1,
  }));
  ...
  const [actThemLichHoc, { loading: loadingThemLichHoc }] = useMutation(
    themLichHocMutation,
    {
      onCompleted: (dataRes) => {
  ...
  const handleButtonOkClick = () => {
    form
      ...
      ?.then(() => {
        ...
        const _thoiGianBatDau = moment(_dataForm?.thoiGianBatDau)?.format(
          "YYYY-MM-DD"
        );
